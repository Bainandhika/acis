...
func NewService(repo Repository, outboxRepo repository.OutboxRepository, db *database.AppDB) Service {
	return &service{
		repo:       repo,
...
func (s *service) CreateDirectTransaction(ctx context.Context, req CreateTransactionDTO) (*TransactionDTO, error) {
...
func (s *service) GetTransactions(ctx context.Context, familyID string) ([]TransactionDTO, error) {
...
func (s *service) CreateProposal(ctx context.Context, req CreateProposalDTO) (*ProposalDTO, error) {
...
func (s *service) ApproveProposal(ctx context.Context, proposalID string, reviewerID string) error {
	tx, err := s.db.BeginTxx(ctx, nil)
	if err != nil {
...
func (s *service) RejectProposal(ctx context.Context, proposalID string, reviewerID string) error {
	tx, err := s.db.BeginTxx(ctx, nil)
	if err != nil {
		return errors.New("failed to start database transaction")
	}
