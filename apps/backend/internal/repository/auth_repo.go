...
func (r *authRepo) SaveOTP(ctx context.Context, email, hashedCode string, expiresAt time.Time) error {
	...
	_, err := r.db.ExecContext(ctx, query, sql.NullString{}, email, hashedCode, expiresAt) // ID will be generated by DB default
	// Note: sqlx doesn't easily support RETURNING with ExecContext for UUID defaults without QueryRowContext.
	// For MVP, we just insert. If you need the ID back, use QueryRowContext like in wallet_repo.
	return err
}
