...
func NewLowBalanceWorker(familyService family.Service, outboxRepo repository.OutboxRepository, db *database.AppDB, interval time.Duration) *LowBalanceWorker {
	if interval <= 0 {
