...
type NotificationHandler func(ctx context.Context, job domain.NotificationJob) error

type WorkerPool struct {
	jobChan     chan domain.NotificationJob
	workerCount int
...
func NewWorkerPool(repo repository.OutboxRepository, workerCount int, chanBufferSize int) *WorkerPool {
	if workerCount <= 0 {
		workerCount = 3
	}
	if chanBufferSize <= 0 {
	...
	return &WorkerPool{
		jobChan:     make(chan domain.NotificationJob, chanBufferSize),
...
func (p *WorkerPool) Submit(job domain.NotificationJob) bool {
	select {
	case p.jobChan <- job:
		return true
	default:
...
func (p *WorkerPool) processJob(ctx context.Context, job domain.NotificationJob, workerID int) {
	p.mu.RLock()
