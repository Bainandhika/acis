<template>
  <section>
    <div class="mb-6 flex items-end justify-between">
      ...
      <div v-if="isAdmin && pending.length" class="rounded bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
        {{ pending.length }} menunggu persetujuan
      </div>
    </div>

    <!-- Member only: Proposal creation form -->
    <form v-if="!isAdmin && family" class="mb-5 grid gap-3 rounded-lg bg-white p-5 shadow-sm md:grid-cols-4"
      ...
      <div>
        ...
        <select id="proposal-type" v-model="proposal.request_type" class="w-full rounded border p-2">
          <option value="add_transaction">Tambah pengeluaran</option>
      ...
      <div class="md:col-span-3">
        ...
        <input id="proposal-description" v-model="proposal.description" placeholder="Deskripsi pengeluaran atau alasan"
          class="w-full rounded border p-2" />
      </div>
    ...
    <div v-else class="space-y-3">
      <article v-for="prop in proposals" :key="prop.id"
        class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 class="font-semibold">{{ prop.title }}</h3>
            <p class="mt-1 text-sm text-slate-500">{{ prop.description || 'Tidak ada deskripsi' }}</p>
          </div>
          <span class="rounded-full px-3 py-1 text-xs font-semibold" :class="statusClass(prop.status)">
            {{ prop.status === 'pending' ? 'Menunggu' : prop.status === 'approved' ? 'Disetujui' : 'Ditolak' }}
          </span>
        </div>
        <div class="mt-4 flex items-center justify-between">
          <strong>{{ money(prop.amount) }}</strong>
          <div v-if="isAdmin && prop.status === 'pending'" class="flex gap-2">
            <button type="button" class="rounded bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700"
              @click="decide(prop.id, true)">Setujui</button>
            <button type="button" class="rounded border border-red-200 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-50"
              @click="decide(prop.id, false)">Tolak</button>
          </div>
        </div>
      </article>
    </div>
...
const proposal = ref({ wallet_id: '', title: '', amount: 0, description: '', request_type: 'add_transaction' })
...
async function load() {
  try {
    const [f, p, w] = await Promise.all([getFamily(), getProposals(), getWallets()])
    family.value = f.data
...
async function submitProposal() {
  try {
    ...
    proposal.value = { wallet_id: '', title: '', amount: 0, description: '', request_type: 'add_transaction' }
    await load()
  } catch (submitError) {
...
async function decide(id, approve) {
  try {
    await (approve ? approveProposal(id) : rejectProposal(id))
    await load()
  } catch (actionError) {
