...
<template>
  <div class="min-h-screen bg-[#F8FAFC] flex flex-col">
    ...
    <main class="flex-1 max-w-[1600px] w-full mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <!-- ========================================================================= -->
      <!-- TAB 1: TWO SIDE-BY-SIDE BLOCKS (LEFT & RIGHT) WITH UNNUMBERED TITLES      -->
      <!-- ========================================================================= -->
      <div v-if="activeTab === 'dashboard'" class="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        
        <!-- ========================================== -->
        <!-- LEFT BLOCK (Col span 7 on large screens)   -->
        ...
        <div class="xl:col-span-7 flex flex-col gap-8">
          ...
          <section class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              ...
              <button 
                @click="openTxModal" 
                class="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition shadow-sm flex items-center gap-1.5 shrink-0"
              >
        ...
        <!-- ========================================== -->
        <!-- RIGHT BLOCK (Col span 5 on large screens)  -->
        ...
        <div class="xl:col-span-5 flex flex-col gap-8">
          ...
          <!-- Family Widget Section (Name, Copyable Invite Code, Vertical Members List) -->
          <section class="flex flex-col gap-4">
            <div>
              <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">
            ...
            <div class="card-neo p-6 flex flex-col gap-5">
              <!-- Top: Large Family Name & Copyable Invite Code -->
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
                <div>
                  <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-brand-100 text-brand-900">
                    {{ t('dashboard.family.title') }}
                  </span>
                ...
                <!-- Copyable Invitation Code with small copy button -->
                <div class="flex flex-col sm:items-end">
                  <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    {{ t('dashboard.family.inviteCodeLabel') }}
                  </span>
                  <div class="inline-flex items-center gap-2 mt-1 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200/90 shadow-sm">
                    ...
                    <button 
                      ...
                      class="p-1 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-200/70 transition active:scale-95"
                      :title="t('nav.copyCode')"
                      type="button"
                    >
                      <svg v-if="!isInviteCodeCopied" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
                        <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
                      </svg>
                      <span v-else class="text-[11px] text-emerald-600 font-black">✓</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div v-if="realFamilyMembers.length === 0" class="py-4 text-center text-xs text-slate-400 font-medium">
                ...
                <div v-else class="flex flex-col gap-2.5">
                  <div 
                    ...
                    :key="member.id"
                    class="p-3 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between gap-3 hover:bg-slate-100/60 transition"
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      ...
                      <div class="min-w-0">
                        <h5 class="text-xs font-extrabold text-slate-900 truncate">
                          {{ t('dashboard.familyMembers.member') }} #{{ member.user_id.slice(0, 6) }}
                        </h5>
    ...
    <dialog :class="isWalletModalOpen ? 'modal modal-open' : 'modal'">
      <div class="modal-box bg-white rounded-3xl p-6 shadow-2xl border border-slate-100 max-w-md">
        ...
        <div class="modal-action mt-6 gap-2">
          <button class="btn btn-ghost btn-sm rounded-xl text-xs font-bold" @click="closeWalletModal" :disabled="isSubmitting">{{ t('modals.createWallet.cancel') }}</button>
          <button class="btn bg-slate-900 hover:bg-slate-800 text-white btn-sm rounded-xl text-xs font-bold border-none" @click="handleSubmitWallet" :disabled="isSubmitting || !newWallet.name">
            {{ isSubmitting ? t('modals.createWallet.saving') : t('modals.createWallet.save') }}
          </button>
        </div>
      </div>
    </dialog>
    ...
    <dialog :class="isTxModalOpen ? 'modal modal-open' : 'modal'">
      <div class="modal-box bg-white rounded-3xl p-6 shadow-2xl border border-slate-100 max-w-md">
        ...
        <div class="modal-action mt-6 gap-2">
          <button class="btn btn-ghost btn-sm rounded-xl text-xs font-bold" @click="closeTxModal" :disabled="isSubmitting">{{ t('modals.createTx.cancel') }}</button>
          <button class="btn bg-slate-900 hover:bg-slate-800 text-white btn-sm rounded-xl text-xs font-bold border-none" @click="handleSubmitTx" :disabled="isSubmitting || !newTx.wallet_id || newTx.amount <= 0">
            {{ isSubmitting ? t('modals.createTx.saving') : t('modals.createTx.save') }}
          </button>
        </div>
      </div>
    </dialog>
    ...
    <dialog :class="isProposalModalOpen ? 'modal modal-open' : 'modal'">
      <div class="modal-box bg-white rounded-3xl p-6 shadow-2xl border border-slate-100 max-w-md">
        ...
        <div class="modal-action mt-6 gap-2">
          <button class="btn btn-ghost btn-sm rounded-xl text-xs font-bold" @click="closeProposalModal" :disabled="isSubmitting">{{ t('modals.proposal.cancel') }}</button>
          <button class="btn bg-slate-900 hover:bg-slate-800 text-white btn-sm rounded-xl text-xs font-bold border-none" @click="handleSubmitProposal" :disabled="isSubmitting || !newProposal.title || newProposal.amount <= 0">
            {{ isSubmitting ? t('modals.proposal.submitting') : t('modals.proposal.save') }}
          </button>
        </div>
      </div>
    </dialog>
